...
(function () {
  ...
  function _samplePoint(xMin, xMax, y, zMin, zMax, maxTries) {
  ...
  function buildPollution(scene) {
    ...
    defs.forEach(function (d, i) {
      var sp = _samplePoint(-22, 20, d.y, -40, -14);
  ...
  function buildAll(scene) {
    ...
    var p;
    p = _samplePoint(-30, 30, -2,  -8, -48); _buildTurtle(p.x, p.y, p.z);
    p = _samplePoint(-30, 30,  1,  -8, -50); _buildJellyfish(p.x, p.y, p.z);
    p = _samplePoint(-30, 30, -1,  -8, -52); _buildClownfish(p.x, p.y, p.z);
    p = _samplePoint(-30, 30,  3, -15, -55); _buildMantaRay(p.x, p.y, p.z);

    // ── 5 bioluminescent eels — spread across a wide seabed band ───────────
    p = _samplePoint(-30, 30, -3,  -8, -50); _buildEel(p.x, p.y, p.z, 0);
    p = _samplePoint(-30, 30, -2, -10, -52); _buildEel(p.x, p.y, p.z, 1);
    p = _samplePoint(-30, 30, -4, -12, -54); _buildEel(p.x, p.y, p.z, 2);
    p = _samplePoint(-30, 30, -1,  -8, -48); _buildEel(p.x, p.y, p.z, 3);
    p = _samplePoint(-30, 30, -3, -10, -55); _buildEel(p.x, p.y, p.z, 4);
